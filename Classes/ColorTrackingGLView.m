...
- (id)initWithFrame:(CGRect)frame 
{
    if ((self = [super initWithFrame:frame])) 
	{
		...
		// Set scaling to account for Retina display if on iPhone ( so we have more than 576 pixels)
		if ([self respondsToSelector:@selector(setContentScaleFactor:)] &&(UI_USER_INTERFACE_IDIOM() != UIUserInterfaceIdiomPad))
		{
...
- (void)setDisplayFramebuffer:(CGRect)frame;
{
    if (context)
    {
        if (!viewFramebuffer)
